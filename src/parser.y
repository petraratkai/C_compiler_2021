...
%type <vexpr> argument_expression_list initializer_list initializer
...
pointer:
		T_MULT 																					{$$ = $1;} //Pointers not implemented
	|	T_MULT type_qualifier_list 																//{$$ = NULL;} //Pointers not implemented
	|	T_MULT pointer 																			//{$$ = $1;} //Pointers not implemented
...
initializer:
		assignment_expression									{std::vector<Expression*>* Initializers = new std::vector<Expression*>; Initializers->push_back($1); $$ = Initializers;}
	|	T_LCURLBRACKET initializer_list T_RCURLBRACKET 			{$$ = $2;}
	|	T_LCURLBRACKET initializer_list T_COMMA T_RCURLBRACKET	{$$ = $2;}
;

initializer_list:
		initializer 											{std::vector<Expression*>* Initializers = new std::vector<Expression*>; Initializers->push_back($1->front()); $$ = Initializers;}

	|	initializer_list T_COMMA initializer 					{$1->push_back($3->front()), $$ = $1;}
;
...
//	|	T_CASE constant_expression T_COLON statement     		{$$ = new CaseStmt($4);} // Don't need to support this, as related to GOTO
//	|	T_DEFAULT T_COLON statement 							//{$$ = new Default;} // Don't need to support this, as realted to GOTO
//;

labeled_statement:
		IDENTIFIER T_COLON statement 							//{$$ = $3;}  // Don't need to support this, as related to GOTO
	|	T_CASE constant_expression T_COLON      				{$$ = new CaseStmt($2);} // Don't need to support this, as related to GOTO
	|	T_DEFAULT T_COLON  										{$$ = new DefaultStmt();} // Don't need to support this, as realted to GOTO
;
...
argument_expression_list:
		assignment_expression																			{std::vector<Expression*>* argList = new std::vector<Expression*>; argList->push_back($1); $$ = argList; }
		| argument_expression_list T_COMMA assignment_expression 				{$1->push_back($3); $$ = $1;} //Fix this
